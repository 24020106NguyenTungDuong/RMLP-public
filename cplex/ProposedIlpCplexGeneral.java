/**
 * Radio Mean Labeling solver: general ILP model via CPLEX (Java Concert
 * Technology), using the paper's proposed literal pairwise-boolean
 * Radio-mean constraint (x[u][l1] + x[v][l2] <= 1 for all u<v,
 * l1+l2 < H(u,v)). Assignment/Injectivity/Span constraints, generic bound
 * UB = n + diam(G) - 1. No symmetry breaking -- this is the general-graph
 * variant; see ProposedIlpCplexCycle.java / ProposedIlpCplexPath.java for
 * the cycle/path-specific symmetry-breaking variants.
 *
 * Build:
 *   javac -cp <CPLEX_STUDIO_DIR>/cplex/lib/cplex.jar ProposedIlpCplexGeneral.java
 * Run:
 *   java -cp .:<CPLEX_STUDIO_DIR>/cplex/lib/cplex.jar \
 *        -Djava.library.path=<CPLEX_STUDIO_DIR>/cplex/bin/x86-64_linux \
 *        ProposedIlpCplexGeneral [timeLimitSeconds] < input.txt
 */
...
public class ProposedIlpCplexGeneral {
