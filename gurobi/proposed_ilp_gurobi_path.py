...
path-specific symmetry breaking (sound only for path graphs, whose sole
non-trivial automorphism is the end-to-end reflection):
...
Run:
GRB_LICENSE_FILE=/path/to/gurobi.lic python3 proposed_ilp_gurobi_path.py < input.txt
"""
...
model = gp.Model("rml_path_literal", env=env)
...
if n > 1:
    model.addConstr(h[0] <= h[n - 1], name="reflection_break")
