"""Independent validation pass over the cycle-graph solvers' results.

Runs the proposed_ilp_gurobi_{general,cycle}.py (Gurobi) and
ProposedIlpCplex{General,Cycle} (CPLEX Java) solvers across datasets/cycle
...
Usage:
    python3 validate_cycle.py [--min N] [--max N] [--timeout SECONDS]
"""
...
CYCLE_DATASET = REPO_ROOT / "datasets" / "cycle"
...
GUROBI_SOLVERS = [
    ...
    ("proposed_ilp_gurobi_cycle", "proposed_ilp_gurobi_cycle.py"),
]
JAVA_SOLVERS = [
    ("ProposedIlpCplexGeneral", "ProposedIlpCplexGeneral"),
    ("ProposedIlpCplexCycle", "ProposedIlpCplexCycle"),
]
...
def instance_files(min_n, max_n):
    files = glob.glob(str(CYCLE_DATASET / "*.txt"))
    out = []
    for p in files:
        name = Path(p).stem
        if not name.isdigit():
...
def main():
    ...
    files = instance_files(args.min, args.max)
    failures = []
    total = 0
